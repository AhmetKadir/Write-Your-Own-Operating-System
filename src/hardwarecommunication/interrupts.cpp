...
uint32_t InterruptManager::DoHandleInterrupt(uint8_t interrupt, uint32_t esp)
{
    ...
    else if(interrupt != hardwareInterruptOffset)
    {
        ...
        // printInt(taskManager->getCurrentTaskPid());
        // printf("\n");
...
int InterruptHandler::fork(CPUState* cpu)
{
    return interruptManager->taskManager->fork(cpu);
}

int InterruptHandler::waitPid(int pid)
{
    return interruptManager->taskManager->waitPid(pid);
}

int InterruptHandler::getPid()
{
    return interruptManager->taskManager->getCurrentTaskPid();
}

int InterruptHandler::Schedule(CPUState* cpu)
{
